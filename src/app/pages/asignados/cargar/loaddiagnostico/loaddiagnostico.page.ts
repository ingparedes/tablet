...
export class LoaddiagnosticoPage implements OnInit {
  ...
  async presentAlertConfirm() {
    const alert = await this.alertController.create({
      header: this._translate.instant('ASIGNADOS-COMPONENT.FOLDER-CARGAR.FOLDER-LOAD-DIAGNOSTICO.DIAGNOSTICOS'),
      message: this._translate.instant('ASIGNADOS-COMPONENT.FOLDER-CARGAR.FOLDER-LOAD-DIAGNOSTICO.MENSAJE'),
      buttons: [
        {
          text: this._translate.instant('ASIGNADOS-COMPONENT.FOLDER-CARGAR.FOLDER-LOAD-DIAGNOSTICO.ENTENDIDO'),
        }
      ]
    });
  ...
  filterPorts(diagnosticos: Diagnostico[], text: string) {
    return diagnosticos.filter(diagnostico => {
      return diagnostico.nombre.toLowerCase().indexOf(text) !== -1 || diagnostico.id.toLowerCase().indexOf(text) !== -1
    });
  }
}
