...
/**
 * Table class for consulta_peru_camas
 */
class consulta_peru_camas extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'consulta_peru_camas';
		$this->TableName = 'consulta_peru_camas';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "\"consulta_peru_camas\"";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id_hospital
		$this->id_hospital = new DbField('consulta_peru_camas', 'consulta_peru_camas', 'x_id_hospital', 'id_hospital', '"id_hospital"', '"id_hospital"', 200, 16, -1, FALSE, '"id_hospital"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id_hospital->Sortable = TRUE; // Allow sort
		$this->fields['id_hospital'] = &$this->id_hospital;

		// nombre_hospital
		$this->nombre_hospital = new DbField('consulta_peru_camas', 'consulta_peru_camas', 'x_nombre_hospital', 'nombre_hospital', '"nombre_hospital"', '"nombre_hospital"', 200, 100, -1, FALSE, '"nombre_hospital"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nombre_hospital->Sortable = TRUE; // Allow sort
		$this->fields['nombre_hospital'] = &$this->nombre_hospital;

		// provincia_hospital
		$this->provincia_hospital = new DbField('consulta_peru_camas', 'consulta_peru_camas', 'x_provincia_hospital', 'provincia_hospital', '"provincia_hospital"', '"provincia_hospital"', 200, 100, -1, FALSE, '"provincia_hospital"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->provincia_hospital->Sortable = TRUE; // Allow sort
		$this->fields['provincia_hospital'] = &$this->provincia_hospital;

		// municipio_hospital
		$this->municipio_hospital = new DbField('consulta_peru_camas', 'consulta_peru_camas', 'x_municipio_hospital', 'municipio_hospital', '"municipio_hospital"', '"municipio_hospital"', 200, 100, -1, FALSE, '"municipio_hospital"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->municipio_hospital->Sortable = TRUE; // Allow sort
		$this->fields['municipio_hospital'] = &$this->municipio_hospital;

		// depto_hospital
		$this->depto_hospital = new DbField('consulta_peru_camas', 'consulta_peru_camas', 'x_depto_hospital', 'depto_hospital', '"depto_hospital"', '"depto_hospital"', 200, 25, -1, FALSE, '"depto_hospital"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->depto_hospital->Sortable = TRUE; // Allow sort
		$this->fields['depto_hospital'] = &$this->depto_hospital;

		// nivel_hospital
		$this->nivel_hospital = new DbField('consulta_peru_camas', 'consulta_peru_camas', 'x_nivel_hospital', 'nivel_hospital', '"nivel_hospital"', '"nivel_hospital"', 200, 100, -1, FALSE, '"nivel_hospital"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nivel_hospital->Sortable = TRUE; // Allow sort
		$this->fields['nivel_hospital'] = &$this->nivel_hospital;

		// sector_hospital
		$this->sector_hospital = new DbField('consulta_peru_camas', 'consulta_peru_camas', 'x_sector_hospital', 'sector_hospital', '"sector_hospital"', '"sector_hospital"', 200, 100, -1, FALSE, '"sector_hospital"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->sector_hospital->Sortable = TRUE; // Allow sort
		$this->fields['sector_hospital'] = &$this->sector_hospital;

		// tipo_zona
		$this->tipo_zona = new DbField('consulta_peru_camas', 'consulta_peru_camas', 'x_tipo_zona', 'tipo_zona', '"tipo_zona"', '"tipo_zona"', 201, 0, -1, FALSE, '"tipo_zona"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->tipo_zona->Sortable = TRUE; // Allow sort
		$this->fields['tipo_zona'] = &$this->tipo_zona;

		// tipo_servicio
		$this->tipo_servicio = new DbField('consulta_peru_camas', 'consulta_peru_camas', 'x_tipo_servicio', 'tipo_servicio', '"tipo_servicio"', '"tipo_servicio"', 201, 0, -1, FALSE, '"tipo_servicio"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->tipo_servicio->Sortable = TRUE; // Allow sort
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "\"consulta_peru_camas\"";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "consulta_peru_camaslist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "consulta_peru_camasview.php")
			return $Language->phrase("View");
		elseif ($pageName == "consulta_peru_camasedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "consulta_peru_camasadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("consulta_peru_camasview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("consulta_peru_camasview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "consulta_peru_camasadd.php?" . $this->getUrlParm($parm);
		else
			$url = "consulta_peru_camasadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("consulta_peru_camasedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("consulta_peru_camasadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("consulta_peru_camasdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->tipo_servicio->setDbValue($rs->fields('tipo_servicio'));
	}
	...
	public function renderListRow()
	{
		...
		$this->tipo_servicio->ViewCustomAttributes = "";
		...
		// tipo_servicio
		$this->tipo_servicio->LinkCustomAttributes = "";
	...
	public function renderEditRow()
	{
		...
		if (!$this->provincia_hospital->Raw)
			$this->provincia_hospital->CurrentValue = HtmlDecode($this->provincia_hospital->CurrentValue);
		$this->provincia_hospital->EditValue = $this->provincia_hospital->CurrentValue;
		$this->provincia_hospital->PlaceHolder = RemoveHtml($this->provincia_hospital->caption());

		// municipio_hospital
		$this->municipio_hospital->EditAttrs["class"] = "form-control";
		$this->municipio_hospital->EditCustomAttributes = "";
		if (!$this->municipio_hospital->Raw)
			$this->municipio_hospital->CurrentValue = HtmlDecode($this->municipio_hospital->CurrentValue);
		$this->municipio_hospital->EditValue = $this->municipio_hospital->CurrentValue;
		$this->municipio_hospital->PlaceHolder = RemoveHtml($this->municipio_hospital->caption());

		// depto_hospital
		$this->depto_hospital->EditAttrs["class"] = "form-control";
		$this->depto_hospital->EditCustomAttributes = "";
		if (!$this->depto_hospital->Raw)
			$this->depto_hospital->CurrentValue = HtmlDecode($this->depto_hospital->CurrentValue);
		$this->depto_hospital->EditValue = $this->depto_hospital->CurrentValue;
		$this->depto_hospital->PlaceHolder = RemoveHtml($this->depto_hospital->caption());
		...
		$this->tipo_zona->PlaceHolder = RemoveHtml($this->tipo_zona->caption());

		// tipo_servicio
		$this->tipo_servicio->EditAttrs["class"] = "form-control";
		$this->tipo_servicio->EditCustomAttributes = "";
		$this->tipo_servicio->EditValue = $this->tipo_servicio->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->tipo_zona);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->tipo_zona);
