...
/**
 * Table class for especial_ambulancia
 */
class especial_ambulancia extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'especial_ambulancia';
		$this->TableName = 'especial_ambulancia';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "\"especial_ambulancia\"";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		$this->id_especialambulancia = new DbField('especial_ambulancia', 'especial_ambulancia', 'x_id_especialambulancia', 'id_especialambulancia', '"id_especialambulancia"', 'CAST("id_especialambulancia" AS varchar(255))', 2, 2, -1, FALSE, '"id_especialambulancia"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id_especialambulancia->IsAutoIncrement = TRUE; // Autoincrement field
		$this->id_especialambulancia->IsPrimaryKey = TRUE; // Primary key field
		$this->id_especialambulancia->Nullable = FALSE; // NOT NULL field
		...
		// especial_es
		$this->especial_es = new DbField('especial_ambulancia', 'especial_ambulancia', 'x_especial_es', 'especial_es', '"especial_es"', '"especial_es"', 200, 60, -1, FALSE, '"especial_es"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->especial_es->Sortable = TRUE; // Allow sort
		$this->fields['especial_es'] = &$this->especial_es;

		// especial_en
		$this->especial_en = new DbField('especial_ambulancia', 'especial_ambulancia', 'x_especial_en', 'especial_en', '"especial_en"', '"especial_en"', 200, 60, -1, FALSE, '"especial_en"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->especial_en->Sortable = TRUE; // Allow sort
		$this->fields['especial_en'] = &$this->especial_en;

		// especial_pr
		$this->especial_pr = new DbField('especial_ambulancia', 'especial_ambulancia', 'x_especial_pr', 'especial_pr', '"especial_pr"', '"especial_pr"', 200, 60, -1, FALSE, '"especial_pr"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->especial_pr->Sortable = TRUE; // Allow sort
		$this->fields['especial_pr'] = &$this->especial_pr;

		// especial_fr
		$this->especial_fr = new DbField('especial_ambulancia', 'especial_ambulancia', 'x_especial_fr', 'especial_fr', '"especial_fr"', '"especial_fr"', 200, 60, -1, FALSE, '"especial_fr"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "\"especial_ambulancia\"";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->id_especialambulancia->setDbValue($conn->getOne("SELECT currval('especial_ambulancia_id_especialambulancia_seq'::regclass)"));
			$rs['id_especialambulancia'] = $this->id_especialambulancia->DbValue;
		}
		return $success;
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('id_especialambulancia', $rs))
				AddFilter($where, QuotedName('id_especialambulancia', $this->Dbid) . '=' . QuotedValue($rs['id_especialambulancia'], $this->id_especialambulancia->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	{
		return "\"id_especialambulancia\" = @id_especialambulancia@";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('id_especialambulancia', $row) ? $row['id_especialambulancia'] : NULL;
		else
			$val = $this->id_especialambulancia->OldValue !== NULL ? $this->id_especialambulancia->OldValue : $this->id_especialambulancia->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@id_especialambulancia@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "especial_ambulancialist.php";
		}
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "especial_ambulanciaedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "especial_ambulanciaadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("especial_ambulanciaview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("especial_ambulanciaview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "especial_ambulanciaadd.php?" . $this->getUrlParm($parm);
		else
			$url = "especial_ambulanciaadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("especial_ambulanciaedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("especial_ambulanciaadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("especial_ambulanciadelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "id_especialambulancia:" . JsonEncode($this->id_especialambulancia->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm != "")
			$url .= $parm . "&";
		if ($this->id_especialambulancia->CurrentValue != NULL) {
			$url .= "id_especialambulancia=" . urlencode($this->id_especialambulancia->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("id_especialambulancia") !== NULL)
				$arKeys[] = Param("id_especialambulancia");
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->id_especialambulancia->CurrentValue = $key;
			else
				$this->id_especialambulancia->OldValue = $key;
